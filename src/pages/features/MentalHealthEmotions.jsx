import React, { useState, useEffect, useRef } from 'react';
 ...
 function MentalHealthEmotions() {
  ...
  const startMeditation = (theme, time) => {
   ...
   setIsPlaying(true);
   setShowMeditationOptions(false);
   setCurrentQuestionIndex(-3); // Status khusus untuk meditasi yang sedang berjalan
  };
  ...
  const endMeditation = () => {
   ...
   setIsFinished(true);
   setShowMeditationOptions(false);
   setCurrentQuestionIndex(-1); // Kembali ke status awal
  };
   ...
   >
     ...
     <div className="grid grid-cols-3 gap-4">
      {meditationDurations.map(d => (
       <motion.button
        ...
        className={`py-2 px-4 rounded-full text-sm font-medium ${duration === d.value ? 'bg-green-500 text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors duration-200'}`}
       >
        {d.label}
       </motion.button>
      ))}
     ...
     <div className="grid grid-cols-2 gap-4">
      {meditationThemes.map(theme => (
       <motion.button
        ...
        className={`py-2 px-4 rounded-full text-sm font-medium ${selectedTheme?.id === theme.id ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700 transition-colors duration-200'}`}
       >
        {theme.name}
       </motion.button>
      ))}
    ...
    {duration && selectedTheme && (
     <motion.button
      ...
      className="bg-gradient-to-r from-[#a78bfa] to-[#818cf8] hover:from-[#818cf8] hover:to-[#a78bfa] text-white font-semibold py-3 px-8 rounded-full transition-colors duration-300 shadow-md hover:shadow-[#a78bfa]/30"
    ...
    <motion.button onClick={() => setCurrentQuestionIndex(-1)} className="mt-4 bg-gray-300 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-semibold py-2 px-6 rounded-full transition-colors duration-300 hover:bg-gray-400 dark:hover:bg-gray-600">
     Batal
    </motion.button>
   </motion.div>
  );
